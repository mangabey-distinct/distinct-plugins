...
**Maximize context understanding.** Most of the value you add is in discovering how the data actually behaves, not in guessing from schema alone. Use `search_references` / `grep_references` for semantic context (repo SQL, prior chats, warehouse knowledge) **and** run exploratory queries so dashboard SQL is based on what you have verified, not assumptions.
...
- **`id`**: For a **new** dashboard, call `allocate_dashboard_id` first and use the returned `dashboard_id` (short lowercase slug plus a random suffix). For **updates**, keep the `id` from `read_dashboard`. Do not invent ids.
...
For a **new** dashboard, call `allocate_dashboard_id` with the same title you will use in the definition, then call `write_dashboard` with that `dashboard_id` as `id`.

If you are **updating** an existing dashboard, always call `read_dashboard` first to get the current state and preserve any elements you are not changing (reuse the existing `id`; do not call `allocate_dashboard_id`).

#### Relevant tools

- `allocate_dashboard_id` -- required once before creating a new dashboard; returns `dashboard_id`
...
class DashboardWriteResponse(BaseModel):
    ...
    url_browser: str = Field(
        ...
        "This is useful in environments where opening the deep link directly is not possible i.e. claude cowork.",
    )
    message: str = Field(..., description="Human-readable success message.")
```

The best way to represent the results of a successful write to the user is to prensent one of the urls to the user. In Claud Cowork (in which you are working) it is not possible for the user to auto-open a deep link, so make sure to allways use the url_browser link. You may style it as a markdown link with the name of the dashboard as the text.

The recommended way is to **always** present it when creating a new dashboard, allowing the user to navigate to the dashborad. But on subsequent writes it is not as important to present the link since the user allready likely has the dashborad open.
