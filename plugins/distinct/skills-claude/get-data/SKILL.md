...
You can never write the final query by just looking at table schemas. Use `search_references` for semantic context (existing SQL files, prior chat conversations, warehouse knowledge snippets) **and** run exploratory queries with `run_query` until your choices are based on verified facts.
